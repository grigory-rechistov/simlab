...
\begin{description}
    ...
    \item[Нет подключения к демону лицензии с того же сервера, на котором запущен lmgrd.] При этом Simics на некоторое время зависает, а затем сообщает код ошибки подключения к серверу лицензий. Проверьте, что настройки Simics указывают на правильный файл.
